...
app.post('/asset', async(request, response)=>{
    // 어플리케이션 요청문서에서 파라미터 꺼내기 (post method에선 body에서 꺼냄)
    const id    = request.body.id;
    const key   = request.body.key;
    const value = request.body.value;
    console.log('/asset-post-' +key+ '-' +value);

    // 인증서 작업 -> registerUser.js 에 있는 'user1' 사용
...
app.get('/asset', async(request, response)=>{
    // 어플리케이션 요청문서에서 파라미터 꺼내기 (get method에선 query에서 꺼냄)
    ...
    // const resultPath = path.join(process.cwd(), '/views/result.html')
    // var resultHTML = fs.readFileSync(resultPath, 'utf8');
    // resultHTML = resultHTML.replace("<div></div>", `<div><p>Transaction has been evlauated: ${txresult}</p></div>`);
    // response.status(200).send(resultHTML);
...
app.get('/asset', async(request, response)=>{
    ...
    resultHTML = resultHTML.replace("<div></div>", `<div><p>Transaction has been evlauated:</p><br> ${tableHTML}</div>\n`);
    response.status(200).send(resultHTML);
    // const obj = JSON.parse(txresult);
    // response.status(200).json(obj);
